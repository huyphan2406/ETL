...
JsonData.select(col("payload.issue.state")).distinct().selectExpr("count(state) as count").show()
JsonData.select(col("payload.issue.state")).distinct().selectExpr("count(*) as count").show()